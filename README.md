...
| [Episode 7](./ep7-citations/) | **Citations & Source Grounding** | Evidence-marked context, clickable citations to exact PDF/text/web evidence, on-demand grounding verification |
...
Advanced features include an Agent Skills system, a code sandbox for executing Python in isolated Docker containers, PII redaction for removing sensitive data, a full autonomous agent platform with Deep Mode for multi-step tasks and a domain-specific harness engine for deterministic workflows (including a contract review harness with DOCX report generation), and a citations layer that grounds every answer in source evidence — clickable chips that jump to the exact bounding box on a PDF, a highlighted passage in extracted text, or a captured web snapshot, with one-click grounding verification.
